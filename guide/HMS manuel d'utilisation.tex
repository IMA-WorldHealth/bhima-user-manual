...
\end{center}
\caption{Illustration d'un sur Clique sur le dossier "admin" afin de pouvoir visualiser ses sous modules}
\label{Illustration d'un sur Clique sur le dossier "admin" afin de pouvoir visualiser ses sous modules}
\end{figure} 
...
Le module Gestion des caisses et des comptes bancaires permet d'administrer les différents types de caisses et des comptes bancaires existants au sein de l'organisation, ce module permet aussi de configurer certains paramètres nécessaire à l'administration des caisses.
...
\section{Analyse du Budget}
L’analyse budgétaire permet de faire l’analyse de l’évolution des principales dépenses et recettes d'une organisation classées par nature. Elle concerne également l’analyse des données classées par secteurs d’activité.

Pour tirer des conclusions intéressantes, comparer ces éléments avec ceux d’autres collectivités, des ratios et des indicateurs peuvent être utilisés même s’ils doivent être maniés avec la plus grande précaution.
\newpage
L'interface principale de ce module se présente de la manière suivante.
\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/AnalyseBudgetaire.png}
\end{center}
\caption{Aperçue de l'interface principale du module Analyse du Budget}
\label{Aperçue de l'interface principale du module Analyse du Budget}
\end{figure}

Pour procéder à l'analyse du budget il suffit de séléctionner le budger rélative à une année fiscale, en suite de déterminer une période ainsi que le type de compte à analysé il peut s'agir soit des dépenses ou bien des ressources, il est aussi possible de pouvoir inclure les budgets des années fiscales précédentes

\begin{figure}[h]
\begin{center}
\includegraphics[width=7cm]{pic/PrecedBudget.png}
\end{center}
\caption{Aperçue du processus d'integration du budget d'une année fiscale antérieure}
\label{Aperçue du processus d'integration du budget d'une année fiscale antérieure}
\end{figure}
\newpage
\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/AnalyseBudgetResult.png}
\end{center}
\caption{Aperçue des données permettant l'analyse budgetaire}
\label{Aperçue des données permettant l'analyse budgetaire}
\end{figure}

\begin{itemize}
\item  \includegraphics[scale=1]{pic/refresh.png} permet de procéder à une nouvelle analyse budgetare,
\item  \includegraphics[scale=1]{pic/ExportCVS.png} permet d'exporter les données aux formats excell,
\item \includegraphics[scale=1]{pic/Print.png} permet d'imprimer les données de l'analyse bugetaire,
\end{itemize}
...
\newpage
\section{Gestion Budgets}
Pour bien établir le budget, le plus facile est de procéder par étape en faisant le point des ressources disponibles ainsi que des dépenses de l'année précédente, en suite établir des prévisions de revenus et de dépenses pour le semestre ou l'année à venir et avec des suivies et analyse régulière du budget ainsi réalisé. Avant d'engager une nouvelle dépense, assurez-vous de disposer des ressources nécessaires.

l'interface principale permettant de d'établir un budget se présente de la manière suivante 

\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/CreateNewBudget.png}
\end{center}
\caption{Aperçue de l'interface principale permettant la création d'un nouveau budget}
\label{Aperçue de l'interface principale permettant la création d'un nouveau budget}
\end{figure}

Pour l'établissement du budget d'un compte, il suffit pour cela de spécifier l'année fiscale dans la liste, il existe deux façon pour créer un budget soit en l'important via Excell ou bien en le définissant manuellement.
\newpage
\subsection{Importation avec Excell}
Pour la création du budget via un fichier excell, il est nécessaire de sélectionner une période de l'année fiscale choisie, après avoir sélectionner la période, un clique sur le bouton \includegraphics[scale=0.7]{pic/EnterNewBudget.png} ouvre une boîte de dialogue permettant de sélectionner un fichier excell au format CVS, la figure suivante donne un apperçue de cette procédure.

\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/LoadCvsBudget.png}
\end{center}
\caption{Aperçue de l'interface illustrant la selection d'un fichier excell au format CVS}
\label{Aperçue de l'interface illustrant la selection d'un fichier excell au format CVS}
\end{figure}

NB, le fichier CVS à importer doit nécessairement être conforme à ceux générer par le système et pour une meilleur utilisation il est souhaitable de d'exporter d'abord le budger de la période pour le quelle on voudrait créer des nouveaux budgets et en suite détérminer le budget des comptes.

Voicu un apercue du fichier CVS permettant l'importation du Budget.


\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/BudgetCVS.png}
\end{center}
\caption{Aperçue d'un fichier CVS compatible pour la création du Budget avec le système}
\label{Aperçue d'un fichier CVS compatible pour la création du Budget avec le système}
\end{figure}

\newpage
\subsection{Définition manuelle du budget} 
La procédure permettant d'établir un budget manuellement, propose une zone de texte permettant la recherche d'un compte pour le quel on voudriait determiner le budget.

\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/CreateMamuelBudget.png}
\end{center}
\caption{Aperçue de l'interface permettant l'établissement manuel du budget d'un compte}
\label{Aperçue de l'interface permettant l'établissement manuel du budget d'un compte}
\end{figure}

Une fois qu'un compte est séléctionné, il suffit de cliquer sur le bouton \textbf{Soumettre} pour preciser le budget d'un compte. Le système signale s'il existe déjà un budget pour ce compte pour l'année fiscale en cours, comme le montre la figure suivante. 

\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/CreatBudgCompte.png}
\end{center}
...
Dans le cas où aucun budget n'existe pour le compte choisi le bouton \includegraphics[scale=0.7]{pic/CreateBudget.png} permet d'acceder à l'interface d'initialisation du budget.
...
Le module Modifier budget d'un compte, permet de spécifier le budget annuel d'un compte, durant une année fiscale, ce module permet aussi de spécifier le budget mensuel d'un compte.
...
\end{figure}
\newpage
Une fois qu'un compte a était choisi, le système redirige directement vers l'interface qui permet de modifier le budget initiale comme le monte la figure suivante.


\begin{figure}[h]
\begin{center}
\includegraphics[width=12cm]{pic/MAJBudget.png}
\end{center}
\caption{Aperçue de l'interface permettant la mise à jour d'un budget}
\label{Aperçue de l'interface permettant la mise à jour d'un budget}
\end{figure}

NB: L'interface permettant de mettre à jour le budget s'actualise automatiquement au moment ou l'on modifie le budget mensuel ou bien annuel d'un compte, pour empecher le budger initiale d'un mois de pouvoir s'ajuster automatiquement il suffit de cliquer sur la case à cochée de la colonne géler rélative à chaque mois comme le montre la figure ci dessus.
La mise à jour du budget d'un compte est effective après un clic sur le bouton \includegraphics[scale=0.7]{pic/UpBudget.png}

\newpage
Mais si le compte choisi ne dispose d'aucun budget le système le signal automatique et donne ainsi la possibilité d'initialisé un nouveau budget
\begin{figure}[h]
\begin{center}
...
\newpage
Pour pouvoir visualiser les détailles d'une année fiscale il suffit de cliquer sur le bouton \includegraphics[scale=1]{pic/EyesBlue.png}.
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=16cm]{pic/GestCentreCout.png}
\end{center}
